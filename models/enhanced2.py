...
# Gracefully handle missing AGE column
if "AGE" in data.columns:
    data["AGE_BIN"] = pd.cut(data["AGE"], bins=[0, 40, 60, 80, 100], labels=[0, 1, 2, 3]).astype(int)
else:
    print("[WARN] AGE column not found, skipping AGE_BIN")
    data["AGE_BIN"] = 1

# Fallback if HB missing
data["HB_LOW"] = (data.get("HB", pd.Series(0)) < 10).astype(int)
...
X_add = data[["AGE_BIN", "HB_LOW"]].values
X_clinical = np.hstack([X_num, X_cat, X_cyto, X_add])
...
clinical_test["AGE_BIN"] = pd.cut(clinical_test["AGE"], bins=[0, 40, 60, 80, 100], labels=[0, 1, 2, 3]).astype(int)
clinical_test["HB_LOW"] = (clinical_test["HB"] < 10).astype(int)
X_add_test = clinical_test[["AGE_BIN", "HB_LOW"]].values
X_clinical_test = np.hstack([X_num_test, X_cat_test, X_cyto_test, X_add_test])
...
gene_vaf_test = molecular_test[molecular_test["GENE"].isin(top_genes)].pivot_table(index="ID", columns="GENE", values="VAF", aggfunc="mean").add_prefix("VAF_")
...
co_occurrence_test = molecular_test[molecular_test["GENE"].isin(["FLT3", "NPM1"])].assign(flag=1).pivot_table(index="ID", columns="GENE", values="flag", fill_value=0)
co_occurrence_test["FLT3_NPM1"] = ((co_occurrence_test.get("FLT3", 0) > 0) & (co_occurrence_test.get("NPM1", 0) > 0)).astype(int)

mol_feats_test = pd.concat([mutation_count_test, avg_vaf_test, gene_flags_test, gene_vaf_test, co_occurrence_test], axis=1).reset_index()
...
# Predict and Save
risk_scores = rsf.predict(X_test_final)
submission = pd.DataFrame({
    "ID": clinical_test["ID"],
    "risk_score": risk_scores
})
submission.to_csv("../submissions/submission_model_enhanced.csv", index=False)
print("✅ Submission saved as submission_model_enhanced.csv")
