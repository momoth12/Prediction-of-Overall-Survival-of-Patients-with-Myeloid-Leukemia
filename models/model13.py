# model4.py: ExtraSurvivalForest + enriched features (translocations, chr7 density, pathway signatures, VAF stats, total anomalies)
...
def count_total_anomalies(text):
    return len(re.findall(r'del|inv|t\(|\+|\-', text.lower()))
...
def extract_cytogenetics_features(df):
    ...
    df["CYTO_COMPLEX_KARYOTYPE"] = cyto_text.apply(lambda s: int(count_total_anomalies(s) >= 3))
    df["N_TRANSLOCATIONS"] = df["CYTOGENETICS"].fillna("").apply(lambda s: count_translocations(s.lower()))
    df["N_TOTAL_ANOMALIES"] = df["CYTOGENETICS"].fillna("").apply(lambda s: count_total_anomalies(s))
    return df
...
X_cyto = data[cyto_cols + ["N_TRANSLOCATIONS", "N_TOTAL_ANOMALIES"]].values
...
vaf_std = molecular.groupby("ID")["VAF"].std().rename("VAF_STD")
vaf_entropy = molecular.groupby("ID")["VAF"].apply(lambda x: -(x / x.sum()).map(lambda p: p*np.log(p+1e-8)).sum()).rename("VAF_ENTROPY")
...
mol_feats = pd.concat([mutation_count, avg_vaf, vaf_std, vaf_entropy, gene_flags], axis=1).reset_index()
...
mol_cols = ['MUTATION_COUNT', 'VAF_AVG', 'VAF_STD', 'VAF_ENTROPY'] + top_genes + pathway_cols
...
print(f"[Eval] IPCW C-index train: {cidx_train:.4f}")
print(f"[Eval] IPCW C-index valid: {cidx_val:.4f}")
...
X_cyto_test = clinical_test[cyto_cols + ["N_TRANSLOCATIONS", "N_TOTAL_ANOMALIES"]].values
...
vaf_std_test = molecular_test.groupby("ID")["VAF"].std().rename("VAF_STD")
vaf_entropy_test = molecular_test.groupby("ID")["VAF"].apply(lambda x: -(x / x.sum()).map(lambda p: p*np.log(p+1e-8)).sum()).rename("VAF_ENTROPY")
...
mol_feats_test = pd.concat([mutation_count_test, avg_vaf_test, vaf_std_test, vaf_entropy_test, gene_flags_test], axis=1).reset_index()
...
submission_dir = "../submissions/submission_model_13_enriched_features.csv"
